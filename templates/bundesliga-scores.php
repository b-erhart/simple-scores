<?php include(__DIR__ . '/../src/Bundesliga/BundesligaScores.php'); ?>
            <nav class="local-nav">
                <div class="local-nav-items">
                    <a <?=$showScores ? 'href="/bundesliga/' . $selectedWeek . '"' : 'class="local-nav-active"'?>>Schedule</a>
                    <a <?=$showScores ? 'class="local-nav-active"' : 'href="/bundesliga/scores/' . $selectedWeek . '"'?>>Scores</a>
...
<?php foreach($matchday->getMatchups() as $matchup): ?>
                <div class="card score">
                    <p class="score-time<?=$matchup->isLive ? ' live' : ''?>"><?=$matchup->isFinished ? 'Final' : ($matchup->isLive ? '● Live' : $matchup->time)?></p>
                    <div class="score-teams">
                        <p class="team-badge score-team-left" style="--team-color: <?=$matchup->homeTeamColor?>"><?=$matchup->homeTeamNameShort?></p>
                        <p class="score-result"><?=($matchup->isFinished || $matchup->isLive) && $showScores ? $matchup->homeTeamScore . ':' . $matchup->awayTeamScore : 'vs.'?></p>
